...
function M.verbose_tip(version)
    if env.VERBOSE then
        return "💡 Verbose mode is enabled; relevant failure output and log paths will be shown.\n"
    end

    return "💡 Tip: \27[93mRun 'PHP_VERBOSE=1 mise install php@" .. (version or "VERSION") .. "'\27[0m to show commands, failure summaries, and log paths.\n"
end
